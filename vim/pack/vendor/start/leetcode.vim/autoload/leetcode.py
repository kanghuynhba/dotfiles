...
def load_manual_session_cookie():
    """Load session cookie from environment variable, file, or keyring"""
    import os
    global session
    session = requests.Session()
    # Try environment variable first
    cookie_value = os.environ.get('LEETCODE_SESSION_COOKIE')
    # Try reading from file
    if not cookie_value:
        cookie_file_paths = [
            os.path.expanduser('~/.leetcode_session'),
            os.path.expanduser('~/.config/leetcode/session'),
            '/tmp/leetcode_session'
        ]
    ...
    if not cookie_value and keyring is not None:
        try:
            cookie_value = keyring.get_password('leetcode.vim', 'MANUAL_SESSION_COOKIE')
        except:
            pass
    if not cookie_value:
        _echoerr('''No session cookie found. Please set one of:
1. Environment variable: export LEETCODE_SESSION_COOKIE="your_cookie_here"
2. File: echo "your_cookie_here" > ~/.leetcode_session
3. Use set_manual_cookie() function to store in keyring''')
        return False
    # Set the cookie in session
    domain = LC_BASE.split('/')[-1]  # 'leetcode.com' or 'leetcode-cn.com'
    session.cookies.set('LEETCODE_SESSION', cookie_value, domain=domain, path='/')
    # Test the cookie
    progress = get_progress()
    if progress is None:
        # Clear expired cookie from keyring
        if keyring is not None:
            try:
                keyring.delete_password('leetcode.vim', 'MANUAL_SESSION_COOKIE')
            except:
                pass
        _echoerr('Session cookie has expired. Please update with a fresh cookie.')
        return False
    return True

def set_manual_cookie(cookie_value, use_keyring=True):
    """Helper function to store manual cookie"""
    import os
    # Store in keyring if available and requested
    if use_keyring and keyring is not None:
        try:
            keyring.set_password('leetcode.vim', 'MANUAL_SESSION_COOKIE', cookie_value)
            print("Cookie stored in keyring successfully")
            return True
        except Exception as e:
            print(f"Failed to store in keyring: {e}")
    # Store in file as fallback
    try:
        cookie_file = os.path.expanduser('~/.leetcode_session')
        with open(cookie_file, 'w') as f:
            f.write(cookie_value)
        os.chmod(cookie_file, 0o600)  # Secure file permissions
        print(f"Cookie stored in {cookie_file} successfully")
        return True
    except Exception as e:
        print(f"Failed to store cookie in file: {e}")
        return False
...
def _get_category_problems(category):
    ...
    for p in content['stat_status_pairs']:
        ...
        if total_submitted > 0:
            ac_rate = p['stat']['total_acs'] / total_submitted
        else:
            ac_rate = 0.0
