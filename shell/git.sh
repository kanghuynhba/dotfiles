...
grd() {
    if ! git rev-parse --is-inside-work-tree >/dev/null 2>&1; then
        echo "Error: Not a git repository." [cite: 119]
        return 1 [cite: 119]
    fi
    
    # We pass the options directly to avoid expansion issues
    if [ $# -eq 1 ]; then
        git difftool -y --extcmd=vimdiff HEAD -- "$1" 
    elif [ $# -eq 2 ]; then
        git difftool -y --extcmd=vimdiff "$1" -- "$2" 
    else
